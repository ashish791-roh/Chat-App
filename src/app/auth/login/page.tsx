...
export default function LoginPage() {
  ...
  /**
   * ✅ Auth Sync (Prevents showing login if already logged in)
   */
  useEffect(() => {
    if (!authLoading && user) {
      router.replace("/");
  ...
  /**
   * ✅ Email Login (Mock or future backend)
   */
...
const handleEmailLogin = async (e: React.FormEvent) => {
  ...
  return (
    <div className="bg-white/70 dark:bg-slate-900/70 backdrop-blur-xl border border-white/20 dark:border-slate-800 rounded-[2.5rem] p-8 shadow-2xl">
      ...
      {/* ✅ ERROR UI */}
      {error && (
        <div className="mb-4 p-3 text-sm text-red-600 bg-red-50 dark:bg-red-900/20 rounded-xl">
          {error}
        </div>
      )}
      ...
      <form className="space-y-5" onSubmit={handleEmailLogin}>
        ...
        <div className="flex items-center gap-2 px-1">
          <input
            ...
            className="rounded-md border-gray-300 text-blue-600 w-4 h-4"
          />
          <label
            htmlFor="remember"
        ...
        <button
          disabled={loading}
          className="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-4 rounded-2xl shadow-lg shadow-blue-500/30 flex items-center justify-center gap-2 transition-all active:scale-95 disabled:opacity-50"
        >

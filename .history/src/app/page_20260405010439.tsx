...
export default function ChatPage() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      const usersList = snapshot.docs
        .map(doc => ({ uid: doc.id, displayName: doc.data().name, ...doc.data() }))
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      const chatList = snapshot.docs.map(doc => ({
        id: doc.id,
        ...doc.data(),
        name: doc.data().isGroup ? doc.data().name : (doc.data().names?.find((n: string) => n !== currentUser.displayName) || "Chat")
      }));
      setChats(chatList);
      if (chatList.length > 0 && !activeChat) setActiveChat(chatList[0]);
    });
    return () => unsubscribe();
  }, [currentUser?.uid, currentUser?.displayName]);

  // 3. Real-time message listener for active chat
  useEffect(() => {
    if (!currentUser?.uid || !activeChat?.id) return;
    const unsubscribe = subscribeToMessages(currentUser.uid, activeChat.id, (msgs) => {
      setMessages(msgs);
    });
    return () => unsubscribe();
  ...
  useEffect(() => {
    ...
    socket.on("typing", (data) => {
      if (data.to === authUser?.id || data.to === activeChat?.id) setIsOtherUserTyping(true);
    });
    socket.on("stop_typing", (data) => {
      if (data.to === authUser?.id || data.to === activeChat?.id) setIsOtherUserTyping(false);
    });
    return () => {
      socket.off("typing");
  ...
  const handleInputChange = (val: string) => {
    ...
    if (val.length > 0) socket.emit("typing", { user: currentUser.displayName, to: activeChat.id });
    else socket.emit("stop_typing", { user: currentUser.displayName, to: activeChat.id });
  };
    ...
    const interval = setInterval(() => {
      ...
      if (progress >= 100) {
        ...
        const fileMessage: Message = {
          id: Date.now().toString(), senderId: currentUser.uid, senderName: currentUser.displayName,
          text: file.type.startsWith('image/') ? URL.createObjectURL(file) : `📄 ${file.name}`,
          timestamp: new Date().toISOString(), isMe: true, status: 'sent'
        };
  ...
  const handleGifSend = (url: string) => {
    ...
    const gifMessage: Message = { 
      id: Date.now().toString(), senderId: currentUser.uid, senderName: currentUser.displayName, 
      text: url, timestamp: new Date().toISOString(), isMe: true, status: 'sent' 
    };
  ...
  if (loading || !currentUser) return <div className="h-screen w-full flex items-center justify-center bg-white dark:bg-slate-950 text-blue-600 font-bold">Loading BlinkChat...</div>;
  ...
  return (
    <main className="flex h-screen bg-white dark:bg-slate-950 overflow-hidden text-gray-900 dark:text-gray-100 transition-colors relative">
      ...
      {isModalOpen && <CreateGroupModal friends={friends} currentUser={currentUser} onClose={() => setIsModalOpen(false)} onCreate={(groupData: any) => {
      ...
      <aside className="w-80 border-r border-gray-200 dark:border-slate-800 hidden md:flex flex-col bg-gray-50 dark:bg-slate-900/50">
        <div className="p-5 border-b dark:border-slate-800 flex items-center justify-between bg-white dark:bg-slate-900">
          <div className="flex items-center gap-2 text-blue-600 dark:text-blue-400 font-bold text-xl"><Users size={20} className="bg-blue-600 p-1.5 rounded-lg text-white" /> BlinkChat</div>
        ...
        <div className="flex-1 overflow-y-auto space-y-1 bg-white dark:bg-slate-900">
          {chats.map((chat) => (
            <div key={chat.id} onClick={() => setActiveChat(chat)} className={cn("flex items-center gap-3 p-4 cursor-pointer transition-all border-l-4", activeChat?.id === chat.id ? "bg-blue-50 dark:bg-slate-800 border-blue-600" : "hover:bg-gray-50 dark:hover:bg-slate-800/50 border-transparent")}>
              <div className={cn("w-12 h-12 rounded-full flex items-center justify-center text-white font-bold shadow-sm", chat.isGroup ? "bg-gradient-to-br from-orange-400 to-rose-500" : "bg-blue-600")}>{chat.isGroup ? <Users size={20} /> : chat.name.charAt(0)}</div>
              <div className="flex-1 min-w-0">
                <div className="flex justify-between items-center"><h3 className="text-sm font-bold truncate">{chat.name}</h3><span className="text-[10px] text-gray-400">12:45</span></div>
          ...
          <>
            <header className="h-[73px] px-4 md:px-6 border-b border-gray-200 dark:border-slate-800 flex items-center justify-between bg-white/80 dark:bg-slate-900/80 backdrop-blur-md z-20">
              <div className="flex items-center gap-3 cursor-pointer" onClick={() => setIsMediaOpen(true)}>
                <div className="relative">
                  <div className={cn("w-10 h-10 rounded-full flex items-center justify-center text-white font-bold shadow-md", activeChat.isGroup ? "bg-gradient-to-tr from-orange-400 to-rose-500" : "bg-blue-600")}>{activeChat.isGroup ? <Users size={18} /> : activeChat.name.charAt(0)}</div>
                  {!activeChat.isGroup && <span className="absolute bottom-0 right-0 w-3 h-3 bg-green-500 border-2 border-white dark:border-slate-900 rounded-full"></span>}
                </div>
                <div><h2 className="font-bold text-gray-800 dark:text-white leading-tight">{activeChat.name}</h2><span className="text-[11px] text-green-500 font-bold">{activeChat.status || "Active Now"}</span></div>
              </div>
            ...
            <div ref={scrollRef} className="flex-1 overflow-y-auto p-6 bg-[#F8F9FB] dark:bg-slate-950 space-y-1">
              {messages.map((msg) => (
                <div key={msg.id} className="flex flex-col">
                  {activeChat.isGroup && !msg.isMe && <span className="text-[10px] text-blue-600 font-bold ml-12 mb-1 uppercase tracking-wider">{msg.senderName}</span>}

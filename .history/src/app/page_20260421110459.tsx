...
export default function ChatPage() {
  ...
  const ChatView = () => (
    <section className="flex flex-col h-full min-w-0 flex-1" style={{ background: "var(--bg-deep)" }}>
      ...
      {/* Input Bar */}
      <div className="shrink-0 px-3 sm:px-5 pt-2 pb-[max(0.75rem,env(safe-area-inset-bottom))]"
          ...
          {isRecording ? (
            ...
            <>
              <button type="button" onClick={() => fileInputRef.current?.click()} title="Attach"
                className="w-9 h-9 flex items-center justify-center rounded-xl transition-all hover:bg-white/10 active:scale-90 shrink-0"
              ...
              <button type="button" onClick={() => setShowGif(prev => !prev)} title="GIF"
                className="hidden sm:flex w-9 h-9 items-center justify-center rounded-xl transition-all hover:bg-white/10 active:scale-90 shrink-0"
                style={{ color: "var(--text-muted)" }}>
                <ImagePlay size={18} />
              </button>
              ...
              <button type="button" onClick={() => setIsMediaOpen(true)} title="More"
                className="flex sm:hidden w-9 h-9 items-center justify-center rounded-xl transition-all hover:bg-white/10 active:scale-90 shrink-0"
                style={{ color: "var(--text-muted)" }}>

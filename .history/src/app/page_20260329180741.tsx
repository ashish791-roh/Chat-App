...
import { 
  ...
  Smile, Gift, ChevronLeft, Phone, Video, X, Trash, Trash2 
} from "lucide-react";
...
export default function ChatPage() {
  ...
  const [messages, setMessages] = useState<Message[]>([
    { 
      ...
      senderId: "other", 
      senderName: "Madamji",
      text: "Hii! Try long-pressing this message.", 
  ...
  return (
    <main className="flex h-screen bg-white dark:bg-slate-950 overflow-hidden text-gray-900 dark:text-gray-100 transition-colors relative">
      ...
      {selectedMessage && (
        <div 
          className="fixed inset-0 bg-black/40 backdrop-blur-sm z-[100] flex items-center justify-center p-6 transition-all"
          onClick={() => setSelectedMessage(null)}
        >
          <div 
            className="bg-white dark:bg-slate-900 rounded-3xl w-full max-w-xs overflow-hidden shadow-2xl animate-in fade-in zoom-in-95 duration-200"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="p-6 space-y-3">
              <h3 className="text-center font-bold text-gray-800 dark:text-gray-200 mb-4">Delete Message?</h3>
              
              <button 
                onClick={() => deleteForMe(selectedMessage.id)}
                className="w-full flex items-center justify-between p-3.5 rounded-xl hover:bg-gray-100 dark:hover:bg-slate-800 text-gray-700 dark:text-gray-300 transition-colors group"
              >
                Delete for me <Trash size={18} className="text-gray-400 group-hover:text-gray-600" />
              </button>
              
              {selectedMessage.isMe && (
                <button 
                  onClick={() => deleteForEveryone(selectedMessage.id)}
                  className="w-full flex items-center justify-between p-3.5 rounded-xl hover:bg-red-50 dark:hover:bg-red-900/20 text-red-600 transition-colors"
                >
              ...
              <button 
                onClick={() => setSelectedMessage(null)}
                className="w-full p-3 mt-2 text-sm text-gray-400 font-medium hover:text-gray-600 dark:hover:text-gray-200"
              >
                Cancel
              </button>
            </div>
      ...
      <aside className="w-80 border-r border-gray-200 dark:border-slate-800 hidden md:flex flex-col bg-gray-50 dark:bg-slate-900/50">
        ...
        <div className="p-4 flex-1 overflow-y-auto space-y-2">
          <p className="px-2 text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-4">Recent Chats</p>
          <div className="flex items-center gap-3 p-3 bg-white dark:bg-slate-800 rounded-2xl shadow-sm border dark:border-slate-700 cursor-pointer">
            ...
            <div className="flex-1">
              <p className="text-sm font-semibold">Madamji</p>
              <p className="text-xs text-gray-500 dark:text-gray-400 truncate">Active now</p>
            </div>
          </div>
        </div>
      ...
      <section className="flex-1 flex flex-col h-full bg-white dark:bg-slate-950 relative">
        ...
        <header className="h-[73px] px-4 md:px-6 border-b border-gray-200 dark:border-slate-800 flex items-center justify-between bg-white/80 dark:bg-slate-900/80 backdrop-blur-md z-20">
          <div className="flex items-center gap-2 md:gap-3">
            ...
            <div>
              <h2 className="font-bold text-gray-800 dark:text-white leading-tight">Madamji</h2>
        ...
        <div ref={scrollRef} className="flex-1 overflow-y-auto p-6 bg-[#F8F9FB] dark:bg-slate-950 space-y-1 transition-colors">
          {messages.map((msg) => (
            <ChatBubble 
              ...
              onDeleteOptions={setSelectedMessage}
            />
          ))}
        </div>
        ...
        <div className="p-4 border-t border-gray-200 dark:border-slate-800 bg-white dark:bg-slate-900 relative">
          ...
          {replyingTo && (
            <div className="max-w-4xl mx-auto mb-2 p-3 bg-gray-50 dark:bg-slate-800 rounded-t-xl border-l-4 border-blue-600 flex justify-between items-center animate-in slide-in-from-bottom-2">
              ...
              <button onClick={() => setReplyingTo(null)} className="text-gray-400 hover:text-gray-600 dark:hover:text-white"><X size={18} /></button>
            </div>
          )}
          ...
          <form onSubmit={handleSendMessage} className={cn("max-w-4xl mx-auto flex items-center gap-1 md:gap-2 bg-gray-100 dark:bg-slate-800 p-2 border dark:border-slate-700 transition-all", replyingTo ? "rounded-b-2xl" : "rounded-2xl")}>
            ...
            <button type="submit" disabled={!input.trim()} className="bg-blue-600 text-white p-2.5 rounded-xl hover:bg-blue-700 disabled:opacity-50 shadow-md shadow-blue-500/20"><Send size={18} fill="currentColor" /></button>
          </form>
        </div>
      </section>
    </main>

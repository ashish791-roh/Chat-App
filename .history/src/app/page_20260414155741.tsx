...
export default function ChatPage() {
  ...
  // ✅ FIX: single openMedia state controls MediaPicker visibility
  ...
  useEffect(() => {
    ...
    el.scrollTo({ top: el.scrollHeight, behavior: "smooth" });
  }, [replyingTo, uploadProgress, isOtherUserTyping, openMedia]);
  ...
  const handleUnpinMessage = async () => {
    if (!activeChat) return;
    try {
      await updateDoc(doc(db, "chats", activeChat.id), { pinnedMessage: null });
    } catch (err) {
      console.error(err);
    }
  };
  ...
  // ✅ FIX: handleGifSend properly closes MediaPicker and sends GIF
  ...
  // ✅ FIX: handleEmojiSelect appends emoji to input without closing picker
  ...
  return (
    <main className="flex h-screen overflow-hidden relative z-10">
      ...
      {activeMessage && (
        ...
        >
          <div
            className="absolute glass-card rounded-2xl w-56 overflow-hidden shadow-2xl animate-in zoom-in-95 duration-100"
            style={{
          ...
          >
            <div className="p-1.5 space-y-0.5">
              <button
                onClick={() => { setReplyingTo(activeMessage.message); setActiveMessage(null); }}
              ...
              {/* Delete for Me */}
              <button
                onClick={() => handleDeleteForMe(activeMessage.message)}
                className="w-full flex items-center gap-3 p-2.5 rounded-xl transition-colors hover:bg-white/10 text-sm"
              >
              ...
              {/* Pin Message */}
              <button
                onClick={() => handlePinMessage(activeMessage.message)}
                className="w-full flex items-center gap-3 p-2.5 rounded-xl transition-colors hover:bg-white/10 text-sm"
              >
              ...
              {/* Star Message */}
              <button
                onClick={() => handleStarMessage(activeMessage.message)}
                className="w-full flex items-center gap-3 p-2.5 rounded-xl transition-colors hover:bg-white/10 text-sm"
              >
              ...
              {activeMessage.message.isMe && (
                <button
                  onClick={() => handleDeleteMessage(activeMessage.message)}
      ...
      >
        ...
        <div className="px-5 pt-6 pb-4" style={{ borderBottom: "1px solid var(--border)" }}>
          ...
          {/* Search */}
          <div
            className="relative input-glow rounded-xl border transition-all"
          ...
          {filteredChats.map((chat) => {
            ...
            return (
              <div
                ...
                className={cn(
                  "chat-row flex items-center gap-3 px-4 py-3 cursor-pointer mx-2 rounded-xl",
                  isActive && "chat-row-active"
                )}
              >
        ...
        <div className="p-4" style={{ borderTop: "1px solid var(--border)" }}>
          ...
          <p
            className="text-[9px] font-mono px-2.5 mt-1 truncate select-all"
            style={{ color: "var(--text-muted)" }}
          >
            {currentUser.uid}
          </p>
      ...
      >
              ...
              >
                ...
                <div>
                  ...
                  <div className="flex items-center gap-1.5">
                    ...
                    <span
                      className="text-[11px]"
                      style={{ color: activeChatIsOnline ? "#34d399" : "var(--text-muted)" }}
                    >
                      {activeChatStatus}
                    </span>
                    {activePeerPhone && (
            ...
            {[
              {
                ...
                action: () =>
                  activeChat &&
                  !activeChat.isGroup &&
                  startCall(activeChat.id, activeChat.name, activePeerPhone ?? undefined, false),
                disabled: !canCall,
              ...
              },
              {
                ...
                action: () =>
                  activeChat &&
                  !activeChat.isGroup &&
                  startCall(activeChat.id, activeChat.name, activePeerPhone ?? undefined, true),
                disabled: !canCall,
              ...
              <button
                ...
                className={cn(
                  ...
                  color,
                  disabled ? "opacity-25 cursor-not-allowed" : "hover:bg-white/8 active:scale-95"
                )}
                style={{ color: "var(--text-secondary)" }}
              >
                <Icon size={18} />
        ...
        <div className="flex-1 relative overflow-hidden">
          ...
          {currentActiveChat?.pinnedMessage && (
            <div
              className="absolute top-0 left-0 right-0 z-20 bg-black/40 backdrop-blur-xl px-5 py-2.5 flex items-center justify-between shadow-sm cursor-pointer hover:bg-white/5 transition-colors border-b border-white/10"
              onClick={() => {
              ...
              <div className="flex items-center gap-3 overflow-hidden">
                ...
                <div className="flex flex-col min-w-0">
                  <span className="text-[10px] font-bold uppercase tracking-wider text-violet-400">
                    {currentActiveChat.pinnedMessage.senderName}
                  </span>
                  <span className="text-xs text-white/80 truncate font-medium">
                    {currentActiveChat.pinnedMessage.text}
                  </span>
                </div>
              </div>
              <button
          ...
          >
            ...
            {messages.length === 0 && activeChat && (
              <div className="flex flex-col items-center justify-center h-full py-20 text-center">
                <div
                  className="w-20 h-20 rounded-3xl bg-gradient-to-br from-violet-500/20 to-cyan-500/20 flex items-center justify-center mb-4 shadow-lg"
            ...
            {messages.map((msg, index) => {
              ...
              const getValidDate = (m: any) =>
                m?.timestamp?.toDate ? m.timestamp.toDate() : new Date();
              ...
              return (
                <div key={msg.id} id={`msg-${msg.id}`}>
                  ...
                  <div
                    className={cn(
                      "flex flex-col mb-1",
                      msg.isMe ? "animate-msg-right" : "animate-msg-left"
                    )}
                  >
                    {activeChat?.isGroup && !msg.isMe && (
                      <span
                        className="text-[10px] font-bold uppercase tracking-wider mb-1 ml-1"
                        style={{ color: "var(--accent-2)" }}
                      >
                        {msg.senderName}
                      </span>
              ...
              <div className="flex justify-end mb-2">
                <div className="glass-card p-3 rounded-2xl w-52">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <span className="text-[11px] ml-auto" style={{ color: "var(--accent-2)" }}>
                      {uploadProgress}%
                    </span>
                  </div>
                  <div
        ...
        >
          ...
          {/* ✅ FIX: MediaPicker rendered here, above the input bar, properly wired up */}
          {openMedia && (
            <div className="absolute bottom-full left-0 right-0 mb-2 px-5 z-50">
              <MediaPicker
          ...
          {editingMessage && (
            <div
              className="mb-2 px-4 py-2.5 rounded-xl flex items-center justify-between gap-3"
              style={{
          ...
          <div
            className={cn(
              "flex items-center gap-2 p-2 rounded-2xl border input-glow transition-all",
              (replyingTo || editingMessage) && "rounded-tl-none rounded-tr-none border-t-0"
            )}
            ...
            {isRecording ? (
              <div className="flex items-center flex-1 py-1 px-3 animate-in fade-in slide-in-from-right-4 duration-300">
                ...
                <button
                  ...
                  className="text-gray-400 hover:text-red-400 mr-4 transition-colors p-2"
                  title="Cancel"
                >
                  <Trash2 size={20} />
                </button>
                <button
                  ...
                  className="w-10 h-10 flex items-center justify-center rounded-xl text-white bg-red-500 hover:bg-red-600 transition-all active:scale-90 shadow-lg shadow-red-500/30"
                >
                  <Send size={18} />
                </button>
              </div>
              ...
              <>
                ...
                {[
                  ...
                  {
                    icon: Gift,
                    action: () => { setOpenMedia(false); setIsGiftModalOpen(true); },
                    title: "Gift",
                  },
                  {
                    icon: CoinsIcon,
                    action: () => { if (!activeChat?.isGroup) setIsSendCoinsModalOpen(true); },
                  ...
                  >
                    <Icon
                      size={20}
                      className={title === "Send Coins" ? "text-yellow-500" : ""}
                    />
                  </button>
                ))}

                {/* ✅ FIX: Smile button toggles MediaPicker, shows active state when open */}
                <button
                  ...
                  className={cn(
                    ...
                    openMedia
                      ...
                      : "hover:bg-white/8 text-[var(--text-muted)]"
                  )}
                >
                  <Smile size={20} />
                </button>

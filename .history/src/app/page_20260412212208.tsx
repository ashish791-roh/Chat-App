...
export default function ChatPage() {
  ...
  // Find the handleReaction function inside ChatPage and replace it:
...
// Improved handleInputChange for smooth typing indicators
const handleInputChange = (val: string) => {
  setInput(val);
  if (!currentUser || !activeChat) return;

  if (val.trim().length > 0) {
    socket.emit("typing", { from: currentUser.uid, to: activeChat.id, senderName: currentUser.displayName });
    if (typingTimeoutRef.current) clearTimeout(typingTimeoutRef.current);

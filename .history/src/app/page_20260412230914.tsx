...
const handleReaction = useCallback(async (emoji: string) => {
 ...
 const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
  ...
  // 1. Reset input so it doesn't get stuck
  ...
  try {
    ...
    const storagePath = `chats/${activeChat.id}/${Date.now()}_${fileName}`;
    ...
    uploadTask.on('state_changed', 
      ...
      async () => {
        // 2. THIS PART IS CRITICAL: Wait for the URL
        const downloadURL = await getDownloadURL(uploadTask.snapshot.ref);
        const finalUrl = String(downloadURL); // Force it to be a string
        ...
        await addDoc(collection(db, "chats", chatId, "messages"), {
          ...
          text: finalUrl, // This is now guaranteed to be a valid URL string
        ...
        // 4. Update the sidebar preview
        await updateDoc(doc(db, "chats", chatId), {
          lastMessage: isImage ? "📷 Photo" : `📄 ${fileName}`,
    ...
    console.error("System Upload Failure:", err);
    setUploadProgress(null);
  }
};

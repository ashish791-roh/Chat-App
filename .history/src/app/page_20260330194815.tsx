...
export default function ChatPage() {
  ...
  const handleSubmit = () => {
    ...
    // Pass the data back to your Firebase handler
    onCreate({
      name: groupName,
  ...
  return (
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50">
      <div className="bg-[#1e1e1e] w-96 rounded-lg p-6 shadow-2xl border border-gray-800">
        <h2 className="text-xl font-semibold text-white mb-4">New Group Chat</h2>
        
        <input 
          type="text" 
          placeholder="Group Name"
          className="w-full p-2 mb-4 bg-gray-800 text-white rounded border border-gray-700 focus:outline-none focus:border-blue-500"
          value={groupName}
        ...
        <div className="max-h-48 overflow-y-auto mb-4 custom-scrollbar">
          {friends.map(friend => (
            ...
            >
              ...
              />
              <span className="text-gray-200">{friend.displayName}</span>
            </div>
          ))}
        </div>

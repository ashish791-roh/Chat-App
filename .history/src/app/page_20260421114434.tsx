...
export default function ChatPage() {
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      setUploadError(err instanceof Error ? err.message : "Upload failed. Check Firebase Storage configuration.");
  ...
  return (
    <main className="flex h-screen overflow-hidden relative z-10" style={{ background: "var(--bg-deep)" }}>
      ...
      {/* Upload error toast */}
      {uploadError && (
        <div className="fixed bottom-24 left-1/2 -translate-x-1/2 z-[160] max-w-sm w-[90vw] animate-in slide-in-from-bottom-4 duration-300">
          <div className="flex items-start gap-3 px-4 py-3 rounded-2xl shadow-2xl border border-red-500/30"
            style={{ background: "var(--bg-elevated)" }}>
            <span className="text-xl shrink-0">⚠️</span>
            <div className="flex-1 min-w-0">
              <p className="text-sm font-semibold text-red-400 mb-0.5">Upload failed</p>
              <p className="text-xs text-white/60 break-words">{uploadError}</p>
            </div>
            <button onClick={() => setUploadError(null)} className="shrink-0 text-white/40 hover:text-white/80 transition-colors p-0.5">
              <X size={14} />
            </button>
          </div>
        </div>

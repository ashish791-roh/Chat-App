...
export default function ChatPage() {
  ...
  // Tracks whether user has manually scrolled up (suppresses auto-scroll)
  const userScrolledUpRef = useRef(false);
  // Tracks previous message count so we only auto-scroll on NEW messages
  ...
  // ── Delivered + Seen (read) status listener ────────────────────────────
  ...
  }, [currentUser?.uid, activeChat?.id]); // eslint-disable-line react-hooks/exhaustive-deps

  // ── Reset scroll state when active chat changes ────────────────────────
  useEffect(() => {
    userScrolledUpRef.current = false;
    prevMsgCountRef.current = 0;
    setShowScrollBtn(false);
    // Immediately jump to bottom on chat switch (no smooth needed)
    if (scrollRef.current) {
      scrollRef.current.scrollTop = scrollRef.current.scrollHeight;
    }
  }, [activeChat?.id]);

  // ── Smooth auto-scroll: only fires when messages actually arrive ────────
  useEffect(() => {
    ...
    if (!isNewMessage) return; // re-render without new message → skip

    // Always scroll for OUR OWN messages; scroll for others only if near bottom
    const lastMsg = messages[messages.length - 1];
    const isOwnMsg = lastMsg?.isMe;
    const isNearBottom = el.scrollHeight - el.scrollTop - el.clientHeight < 120;

    if (isOwnMsg || isNearBottom || !userScrolledUpRef.current) {
      el.scrollTo({ top: el.scrollHeight, behavior: "smooth" });
      userScrolledUpRef.current = false;
      setShowScrollBtn(false);
    } else {
      // New message arrived but user is scrolled up — show button
      setShowScrollBtn(true);
    }
  }, [messages]);

  // Also smooth-scroll when typing indicator or emoji picker appears
  useEffect(() => {
    const el = scrollRef.current;
  ...
  // ── Scroll event — detect manual scroll up ─────────────────────────────
  ...
  }, [input, currentUser?.uid, activeChat?.id, replyingTo]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const handleReaction = useCallback(async (emoji: string) => {
    ...
    setActiveMessage(null);
  }, [activeMessage?.id, currentUser?.uid, activeChat?.id]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  // ── Improved typing system — debounced, non-breaking ──────────────────
  ...
  // ── Better Enter key UX ────────────────────────────────────────────────
  // Enter → send; Shift+Enter → newline (textarea); for plain input just send
      ...
      <section className="flex-1 flex flex-col h-full bg-white dark:bg-slate-950 relative">
        ...
        <div className="flex-1 relative overflow-hidden">
          <div
            ...
            className="h-full overflow-y-auto p-6 bg-[#F8F9FB] dark:bg-slate-950 space-y-1 scroll-smooth"
          >
            {messages.map(msg => (
          ...
          {showScrollBtn && (
            <button
              ...
              className="absolute bottom-4 right-4 z-30 bg-blue-600 hover:bg-blue-700 text-white rounded-full w-10 h-10 flex items-center justify-center shadow-lg transition-all active:scale-90 animate-in zoom-in duration-200"

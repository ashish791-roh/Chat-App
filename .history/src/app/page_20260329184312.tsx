...
export default function ChatPage() {
  ...
  useEffect(() => {
    ...
    // If no user is found in storage, send them to login
    if (!user) {
    ...
    <main className="flex h-screen bg-white dark:bg-slate-950 overflow-hidden text-gray-900 dark:text-gray-100 transition-colors relative">
      ...
      <aside className="w-80 border-r border-gray-200 dark:border-slate-800 hidden md:flex flex-col bg-gray-50 dark:bg-slate-900/50">
        ...
        <div className="p-4 border-t dark:border-slate-800 bg-white dark:bg-slate-900">
          <button title="Logout" className="flex items-center justify-center gap-2 text-gray-500 hover:text-red-500 w-full py-2 rounded-xl transition-all"><LogOut size={18} /> Logout</button>
        </div>
      </aside>
      ...
      <section className="flex-1 flex flex-col h-full bg-white dark:bg-slate-950 relative">
        ...
        <header className="h-[73px] px-4 md:px-6 border-b border-gray-200 dark:border-slate-800 flex items-center justify-between bg-white/80 dark:bg-slate-900/80 backdrop-blur-md z-20">
          <div className="flex items-center gap-2 md:gap-3">
            <button title="Go back" className="md:hidden p-2 -ml-2 text-gray-500"><ChevronLeft size={24} /></button>
            <div className="relative">
          ...
          <div className="flex items-center gap-2">
            <button title="Start call" className="p-2 text-gray-400 hover:text-blue-500 hidden sm:block"><Phone size={20} /></button>
            <button title="Start video call" className="p-2 text-gray-400 hover:text-blue-500 hidden sm:block"><Video size={20} /></button>
            <button title="More options" className="p-2 text-gray-400 hover:bg-gray-100 dark:hover:bg-slate-800 rounded-full transition-colors"><MoreVertical size={20} /></button>
          </div>
        </header>
        ...
        <div className="p-4 border-t border-gray-200 dark:border-slate-800 bg-white dark:bg-slate-900 relative">
          ...
          {replyingTo && (
            <div className="max-w-4xl mx-auto mb-2 p-3 bg-gray-50 dark:bg-slate-800 rounded-t-xl border-l-4 border-blue-600 flex justify-between items-center animate-in slide-in-from-bottom-2">
              ...
              <button title="Cancel reply" onClick={() => setReplyingTo(null)} className="text-gray-400 hover:text-gray-600 dark:hover:text-white transition-colors"><X size={18} /></button>
            </div>
          )}
          ...
          <form onSubmit={handleSendMessage} className={cn("max-w-4xl mx-auto flex items-center gap-1 md:gap-2 bg-gray-100 dark:bg-slate-800 p-2 border dark:border-slate-700 transition-all", replyingTo ? "rounded-b-2xl" : "rounded-2xl")}>
            <button type="button" title="Toggle emoji picker" onClick={() => { setShowEmoji(!showEmoji); setShowGif(false); }} className={cn("p-2 rounded-full transition-all", showEmoji ? "text-blue-500 bg-blue-100 dark:bg-blue-900/30" : "text-gray-500 hover:bg-gray-200 dark:hover:bg-slate-700")}><Smile size={22} /></button>
            <button type="button" title="Toggle GIF picker" onClick={() => { setShowGif(!showGif); setShowEmoji(false); }} className={cn("p-2 rounded-full transition-all", showGif ? "text-pink-500 bg-pink-100 dark:bg-pink-900/30" : "text-gray-500 hover:bg-gray-200 dark:hover:bg-slate-700")}><Gift size={22} /></button>
            ...
            <button type="submit" title="Send message" disabled={!input.trim()} className="bg-blue-600 text-white p-2.5 rounded-xl hover:bg-blue-700 disabled:opacity-50 shadow-md shadow-blue-500/20 transition-all active:scale-95"><Send size={18} fill="currentColor" /></button>
          </form>
        </div>
      </section>
    </main>

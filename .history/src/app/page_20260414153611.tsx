...
export default function ChatPage() {
  ...
  { icon: Smile, action: () => { setOpenMedia(true); }, title: "Emoji" },

  { icon: Gift, action: () => setIsGiftModalOpen(true), title: "Gift" },

  { icon: CoinsIcon, action: () => { !activeChat?.isGroup && setIsSendCoinsModalOpen(true); }, title: "Send Coins" },
  ...
  <button
    ...
    onClick={action}
    title={title}
    className="w-9 h-9 flex items-center justify-center rounded-xl hover:bg-white/8"
  >
    <Icon size={20} />
  </button>
))}
                ...
                {openMedia && (
                  <MediaPicker
                    onSelect={(item, type) => {
                      ...
                      if (type === "sticker") {
        // treat sticker like image message
                        handleGifSend(item);
                      }

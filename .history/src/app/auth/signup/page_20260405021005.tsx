...
import { Mail, Lock, ArrowRight, User, AtSign, CheckCircle2, XCircle, Loader2 } from "lucide-react";
...
export default function SignupPage() {
  ...
  useEffect(() => {
    ...
    // Validate: 3–20 chars, letters/numbers/underscores only
    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-50 dark:bg-slate-950 p-4">
      <div className="w-full max-w-md bg-white/70 dark:bg-slate-900/70 backdrop-blur-xl border border-white/20 dark:border-slate-800 rounded-[2.5rem] p-8 shadow-2xl">
        ...
        <form className="space-y-4" onSubmit={handleEmailSignup}>
          ...
          {/* Username */}
          <div className="space-y-2">
            <label className="text-xs font-bold ml-1 text-gray-400 uppercase tracking-wider">Username</label>
            <div className="relative">
              <div className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400"><AtSign size={18} /></div>
              <input required type="text" value={username}
                onChange={(e) => setUsername(e.target.value.replace(/\s/g, ""))}
                placeholder="ashish_rohilla"
                className="w-full pl-12 pr-4 py-3.5 bg-gray-50 dark:bg-slate-800 border-none rounded-2xl focus:ring-2 ring-blue-500 outline-none transition-all dark:text-white" />
            </div>
            {usernameHint && <p className="text-[11px] ml-2">{usernameHint}</p>}
          </div>

          {/* Email */}
          <div className="space-y-2">
            <label className="text-xs font-bold ml-1 text-gray-400 uppercase tracking-wider">Email Address</label>
            <div className="relative">
          ...
          <button disabled={loading || usernameStatus !== "available"} type="submit"

...
import MediaSidebar from "@/components/MediaSidebar"; // New Import
...
export default function ChatPage() {
  ...
  const [isMediaOpen, setIsMediaOpen] = useState(false); // Media Sidebar State
  ...
  // Helper to close active selection when delete menu closes
    ...
    <main className="flex h-screen bg-white dark:bg-slate-950 overflow-hidden text-gray-900 dark:text-gray-100 transition-colors relative">
      ...
      {activeMessage && (
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-[110] flex items-center justify-center p-4 transition-all" onClick={() => setActiveMessage(null)}>
          <div className="bg-white dark:bg-slate-900 rounded-[2.5rem] w-full max-w-xs overflow-hidden shadow-2xl animate-in fade-in zoom-in-95 duration-200" onClick={(e) => e.stopPropagation()}>
      ...
      <aside className="w-80 border-r border-gray-200 dark:border-slate-800 hidden md:flex flex-col bg-gray-50 dark:bg-slate-900/50">
        ...
        <div className="p-4 flex-1 overflow-y-auto space-y-4">
          <div>
            <p className="px-2 text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-3">My Profile</p>
            <div 
              onClick={() => setIsProfileOpen(true)}
              className="group flex items-center gap-3 p-3 bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-transparent hover:border-blue-500 transition-all cursor-pointer"
            >
              ...
              <div className="flex-1">
                <p className="text-sm font-bold">Ashish Yadav</p>
          ...
          <div>
            ...
            <div className="flex items-center gap-3 p-3 bg-white dark:bg-slate-800 rounded-2xl shadow-sm border dark:border-slate-700 cursor-pointer hover:bg-gray-50 dark:hover:bg-slate-700/50">
              <div className="relative">
                <div className="w-11 h-11 bg-gradient-to-tr from-blue-500 to-purple-500 rounded-full flex items-center justify-center text-white font-bold">A</div>
      ...
      <section className={cn(
        "flex-1 flex flex-col h-full bg-white dark:bg-slate-950 relative transition-all duration-300",
        isMediaOpen && "mr-0 md:mr-0" // The sidebar floats over on mobile, pushes on desktop if desired
      )}>

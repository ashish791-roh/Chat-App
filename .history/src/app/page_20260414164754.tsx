...
export default function ChatPage() {
  ...
  return (
    <main className="flex h-screen overflow-hidden relative z-10">
      ...
      {showGif && ( <GifPicker isOpen={showGif} onClose={() => setShowGif(false)} onSelect={(gifUrl: string) => { handleGifSend(gifUrl); setShowGif(false); }} />)}
      {showEmoji && (
  <div className="absolute bottom-24 left-5 z-50 bg-black/80 backdrop-blur-xl p-3 rounded-2xl grid grid-cols-8 gap-2 shadow-2xl">
    {["😀","😂","😍","😭","😎","👍","🔥","🎉","❤️","😡","🙏","😴"].map((emoji) => (
      <button
        key={emoji}
        onClick={() => {

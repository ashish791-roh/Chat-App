...
interface UseCallOptions {
  ...
  onCallEnded?: (details: { peerId: string; peerName: string; duration: number; isVideo: boolean; status: "completed" | "missed" | "declined", direction: "incoming" | "outgoing" }) => void;
}
...
export function useCall({
  ...
  const endCall = useCallback(
    (reason: "end" | "decline" = "end") => {
      if (activeCall) {
        ...
        if (duration === 0) {
           status = reason === "decline" ? "declined" : "missed";
        }
    ...
    [activeCall, cleanup, onCallEnded]
  );
  ...
  useEffect(() => {
    ...
    const onEnd = (data?: { reason: string }) => {
       if (activeCall && onCallEnded) {
         const duration = startTimeRef.current ? Math.floor((Date.now() - startTimeRef.current) / 1000) : 0;
         let status: "completed" | "missed" | "declined" = "completed";
         if (duration === 0) {
           status = data?.reason === "decline" ? "declined" : "missed";
         }
         onCallEnded({
           peerId: activeCall.peerId,
           peerName: activeCall.peerName,

...
const STALL_CHECK_MS = 10_000; // check every 10 s; cancel after 3 consecutive stalls (30 s)
...
export function uploadToStorage(
  ...
  return new Promise((resolve, reject) => {
    ...
    // Track whether WE initiated the cancel (stall), so we can suppress
    // the redundant storage/canceled error that fires right after.
    let cancelledByStall = false;
    let stallCount = 0;
    let lastBytes  = -1;
    ...
    const stallInterval = setInterval(() => {
      ...
      if (current === lastBytes) {
        if (++stallCount >= 3) {
          ...
          task.cancel(); // triggers error callback with storage/canceled — we suppress it below
          reject(new Error(
            "Upload stalled — Firebase Storage may not be configured.\n" +
            "Check: (1) storageBucket in .env.local is set, " +
    ...
    task.on(
      ...
      (err) => {
        clearInterval(stallInterval);
        // Suppress the redundant error that fires when WE cancel due to stall
        if (cancelledByStall) return;
        ...
        if (code === "storage/unauthorized") {
          ...
          // Don't log user-cancel as an error
          console.error("[upload] Storage error:", code, err.message);
        }
        reject(err);
      },
      async () => {
...
export async function uploadFile(
  ...
  try {
    ...
    // Use explicit prefixes so ChatBubble never misidentifies content type
  ...
  // ── Base64 fallback (small files only) ───────────────────────────────────
  if (uploadBlob.size / 1024 > MAX_BASE64_KB) {
    throw new Error(
      `File too large for offline fallback (${(uploadBlob.size/1024).toFixed(0)} KB > ${MAX_BASE64_KB} KB).\n` +
      "Please fix your Firebase Storage configuration so files can upload directly."
    );
  }

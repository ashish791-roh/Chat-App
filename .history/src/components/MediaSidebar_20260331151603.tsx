...
export default function ProfileModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  ...
  useEffect(() => {
    if (user) {
      ...
      setBio(user.bio || "Exploring Next.js 🚀");
    }
  }, [user, isOpen]);
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      await updateUserProfile(user.id, { name, avatar: photo || "", bio });
      onClose();
      // Force a reload to ensure all components pull from the updated localStorage/Auth
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[150] flex items-center justify-center p-4">
      <div className="bg-white dark:bg-slate-900 w-full max-w-md rounded-[2.5rem] shadow-2xl border border-white/20 overflow-hidden">
        ...
        <div className="px-8 pb-8 -mt-12 relative text-center">
          <div className="relative inline-block group">
            <div className="w-24 h-24 rounded-3xl bg-white dark:bg-slate-800 p-1 shadow-xl overflow-hidden">
               {photo ? <img src={photo} alt="Profile" className="w-full h-full object-cover rounded-[1.2rem]" /> : <div className="w-full h-full rounded-[1.2rem] bg-gray-100 dark:bg-slate-700 flex items-center justify-center text-3xl font-bold text-blue-600">{name[0]}</div>}
            </div>
          ...
          <button onClick={handleSave} disabled={saving} className="w-full mt-8 bg-blue-600 hover:bg-blue-700 text-white font-bold py-4 rounded-2xl shadow-lg flex items-center justify-center gap-2 transition-all active:scale-95 disabled:opacity-50">
            {saving ? <Loader2 className="animate-spin" /> : <Check size={20} />} Save Changes
          </button>
        </div>
      </div>
    </div>

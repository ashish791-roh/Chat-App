...
export default function ChatBubble({ 
  ...
  onReply, 
  onActionMenu,
  onPreview // NEW: Added prop for triggering the file preview modal
}: { 
  message: Message, 
  onReply: (msg: Message) => void,
  onActionMenu: (msg: Message) => void,
  onPreview: (url: string) => void // NEW: Type definition for the preview function
}) {
  const [formattedTime, setFormattedTime] = useState("");
  const timerRef = useRef<NodeJS.Timeout | null>(null);

  // CORRECTED LOGIC: Expanded to detect blob previews, common image types, and explicit image types
  const isMedia = 

...
export default function ChatBubble({
  ...
  // IMG:: prefix — explicit image marker (set by uploadFile)
  if (contentText.startsWith("IMG::")) {
    isImage     = true;
    contentText = contentText.slice(5); // strip prefix, rest is URL or base64
  }
  // FILE:: prefix
  ...
  // Legacy: raw base64 image (older messages stored without prefix)
  else if (contentText.startsWith("data:image/")) {
    isImage = true;
  }
  // Legacy: raw base64 audio
  else if (contentText.startsWith("data:audio/")) {
    isAudio = true;
  }
  // Legacy: any other base64 blob
  else if (contentText.startsWith("data:")) {
    isPdfOrFile = true;
  }

  // Explicit fileType override from message metadata
  ...
  const handleDownload = (e: React.MouseEvent) => {
    ...
    a.href     = contentText;
    a.download = isImage ? (fileName || "image.jpg") : fileName;
    a.target   = "_blank";
          ...
          {message.isDeleted ? (
            ...
            <div className="relative group cursor-pointer flex" onClick={() => setShowImageModal(true)}>
              <img
                ...
                alt={fileName}
                onError={(e) => {
                  // Show broken-image placeholder instead of blank
              ...
              <button
                ...
                className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity bg-black/50 hover:bg-black/70 text-white p-1.5 rounded-full"
              >
                <Download size={14} />
              </button>
            </div>
            ...
            <div className="flex items-center gap-2 p-1">
              ...
              <div className="flex flex-col min-w-0">
                ...
                <button
                  ...
                  className="text-xs underline opacity-70 hover:opacity-100 text-left mt-0.5"
                >
                  Download
                </button>
              </div>

...
export default function ChatBubble({ message, onReply, onActionMenu }: any) {
  ...
  const isImage = message.text?.match(/\.(jpeg|jpg|gif|png|webp)$/i) || message.text?.startsWith("blob:");
  const isPdf = message.text?.toLowerCase().includes(".pdf") || message.text?.startsWith("📄");
    ...
    <div className={cn("flex w-full mb-1 group", isMe ? "justify-end" : "justify-start")}
      ...
      <div className={cn(
        ...
        <div className="flex flex-col gap-1">
          {isImage ? (
            <div className="relative group/img cursor-pointer" onClick={() => window.open(message.text, '_blank')}>
              <img src={message.text} className="rounded-lg max-h-72 w-full object-cover border border-white/10" alt="Shared Image" />
            ...
            <div className="flex items-center gap-3 bg-black/20 p-3 rounded-xl border border-white/5 cursor-pointer hover:bg-black/30 transition-colors"
                 onClick={() => {
                   const url = message.text.replace("📄 ", "");
              ...
              <div className="flex-1 min-w-0">
                <p className="text-sm font-medium truncate">{message.text.split('/').pop() || "Document.pdf"}</p>

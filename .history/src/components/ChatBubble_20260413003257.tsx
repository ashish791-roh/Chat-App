...
export default function ChatBubble({ message, onReply, onActionMenu }: any) {
  ...
  // So we must check fileType first, then fall back to URL parsing.
  const isImage =
    fileType === "image" ||
    (fileType === "" &&
      ...
      (() => {
        try {
          // Decode the URL so encoded paths like %2F become /
      ...
      >
        <div className="flex flex-col gap-1">
          ...
          {isImage && text.startsWith("http") ? (
            ...
            /* ── PDF ── */
            <div
              className="flex items-center gap-3 bg-black/30 p-3 rounded-xl border border-white/5 cursor-pointer hover:bg-black/50 transition-colors mt-1"
              onClick={() => window.open(text, "_blank", "noopener,noreferrer")}
            >
